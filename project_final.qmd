...
grid.arrange(pplot_linear, pplot_normal, pplot_default, pplot_cauchy, ncol=2, nrow = 2) 
```

```{r}
#normal prior
nonlinear_model_normal_prior <- stan_glm(formula_linear, data = cath,
                family = binomial(link = "logit"),
                prior = normal_prior, prior_intercept = normal_prior,
                QR=TRUE, refresh=0)

#default prior
nonlinear_model_default_prior <- stan_glm(formula_linear, data = cath,
                family = binomial(link = "logit"),
                QR=TRUE, refresh=0)

#cauchy prior
nonlinear_model_cauchy_prior <- stan_glm(formula_linear, data = cath,
                family = binomial(link = "logit"),
                prior = cauchy_prior, prior_intercept = cauchy_prior,
                QR=TRUE, refresh=0)
```
