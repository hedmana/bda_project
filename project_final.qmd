...
Running the non linear model with the new set of priors:
```{r}
...
Due to the nature of the nonlinear model, it makes more sense to inspect the Rhat and ESS values:

```{r}
#linear prior sensitivity analysis plots
nonlinear_model_default_prior$stan_summary[, "Rhat"] %>% round(3)
```
